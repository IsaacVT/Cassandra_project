...
export default function ProductGridDetails({ product }) {
  ...
  return (
    <>
      ...
      <Grid item xs={6}>
        <Stack spacing={3}>
          <Typography sx={{ fontSize: 'h2.fontSize' }}>{name}</Typography>

          <Typography sx={{ fontSize: 'h4.fontSize' }}>{description}</Typography>

          <Stack spacing={1}>
            <Stack direction={'row'} spacing={1}>
              <Typography sx={{ fontSize: 'h5.fontSize', color: 'success.dark', fontWeight: 'bold' }}>
                Price now: {fCurrency(price)}
              </Typography>
